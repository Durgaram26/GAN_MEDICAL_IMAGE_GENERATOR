...
os.makedirs("generated_images", exist_ok=True)  # Folder to save generated images
...
uploaded_images = []  # List of file paths for uploaded images
trained_models = {}
training_progress = {}  # Global progress dictionary for real-time updates

##############################################
# Model Definitions: Generative Models
##############################################
...
##############################################
# Model Definitions: Super-Resolution Models
##############################################
...
def train_model(model_type, image_paths, epochs, batch_size, lr, latent_dim=100, upscale_factor=2,
    ...
    if model_type in ["GAN", "StyleGAN", "StyleGAN2 ADA"]:
        ...
        if model_type == "GAN":
            ...
            netG = StyleGAN2ADAGenerator(latent_dim=latent_dim, feature_maps=feature_maps).to(device)
...
def get_generator_instance(model_type, latent_dim):
    if model_type == "GAN":
        ...
        return StyleGAN2ADAGenerator(latent_dim=latent_dim)
        ...
        for name, data in metadata.items():
            ...
            if generator:
                ...
                if os.path.exists(generator_path):
                    state = torch.load(generator_path, map_location=torch.device("cpu"))
                    ...
                    if filename.lower().endswith(".zip"):
                        ...
                        for root, _, files_in_zip in os.walk(extract_dir):
                            for f in files_in_zip:
                                # Only add to uploaded_images if not already present
...
@app.route("/progress")
def progress():
    # Return the current training progress as JSON
